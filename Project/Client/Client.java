...
public class Client {
    ...
    private boolean connect(String address, int port) {
        try {
            ...
            System.out.println("[info] connected to " + address + ":" + port);
...
//Karenralda //Kar65 //11/04/25
        if (isConnection(text)) {
            ...
            //Kar65 //Karenralda //Nov032025
            clientName = text.substring(6).trim();
            System.out.println("[info] name set to " + clientName);
            wasCommand = true;

        } else if (text.toLowerCase().startsWith("/createroom ")) {
        String room = text.substring(12).trim();
        if (room.isEmpty()) {
            System.out.println("Usage: /createroom <roomName>");
        } else {
            String[] cmd = { Constants.COMMAND_TRIGGER, "createroom", room };
            sendToServer(String.join(",", cmd));
            System.out.println("[info] requested room create: " + room);
        }
        wasCommand = true;

        
} else if (text.toLowerCase().startsWith("/joinroom ")) {
    String room = text.substring(10).trim();
    if (room.isEmpty()) {
        System.out.println("Usage: /joinroom <roomName>");
    } else {
        String[] cmd = { Constants.COMMAND_TRIGGER, "joinroom", room };
        sendToServer(String.join(",", cmd));
        System.out.println("[info] requested join: " + room);
    }
    wasCommand = true;

} else if ("/leaveroom".equalsIgnoreCase(text)) {
    String[] cmd = { Constants.COMMAND_TRIGGER, "leaveroom" };
    sendToServer(String.join(",", cmd));
    System.out.println("[info] requested leave room (to lobby)");
    wasCommand = true;
